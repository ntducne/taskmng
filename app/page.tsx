...
import { Calendar, DateField, DatePicker, AlertDialog, Button, Checkbox, CheckboxGroup, Chip, Dropdown, FieldError, Form, Header, Input, InputGroup, Label, Modal, Pagination, Spinner, Table, TextField } from "@heroui/react";
...
export default function Home() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      task_status: selected.length > 0 ? Number(selected[selected.length - 1]) : 1,
    };
    try {
      let response = null;
      if (detailItem) {
  ...
  return (<>
          ...
          <div className="grid grid-cols-2">
            ...
            <div className="flex justify-end items-end gap-3">
              <div className="flex gap-2 border-r pr-4 border-gray-200">
                <InputGroup className="border border-purple-500 rounded-md ring-0">
                  <InputGroup.Prefix>
                ...
                <Button size="md" className="px-4 bg-purple-500 text-white rounded-md" onClick={() => {
                  getData();
                }}>Search</Button>
              </div>
              <Button size="md" className="px-4 bg-teal-500 text-white rounded-md" onPress={() => openDialogCreate()}>Create Task</Button>
            </div>
          </div>
    ...
    <Modal.Backdrop isOpen={isOpen} onOpenChange={setIsOpen} className={animations.backdrop} isDismissable={false} isKeyboardDismissDisabled={true}>
      <Modal.Container size='lg' className={animations.container}>
        <Modal.Dialog>
          <Modal.Header>
            <Modal.Heading>{detailItem ? "Edit Task " + detailItem.task_id : "Create New Task"}</Modal.Heading>
          </Modal.Header>
            ...
            <Form onSubmit={onSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <DatePicker className="w-full mt-3" name="deploy_date" value={defaultItem?.deploy_date ? parseDate(defaultItem.deploy_date) as any : undefined}>
                    <Label>Date</Label>
                  ...
                  <TextField name="bug" type="text" className="mt-3">
                    <Label>Total Bug</Label>
                    <Input value={defaultItem.bug} />
                  </TextField>
                </div>
              </div>
